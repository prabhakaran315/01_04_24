...
try:
    ...
    selected_font.set(font_families[138])  # Set the default value to the first font in the list
    # ----------------Font Style Ended -----------------#
        ...
        if (astranotebook.index(astranotebook.select()) == 3):
            ...
            try:
                if low_pf_entry.get() and high_pf_entry.get() and grid_kw_entry.get() and panel_rating_combo.current() != 0:
                    ...
                    if font_style == True:
                        font_frame = Frame(astrap4_frame, bg="red", width = 255, height = 60)
                        font_frame.place(x=450, y=200)
                        select_btn = Button(font_frame, text = "Select", bg=background_color, font = ("Verdana", 10), command = true_pf_pdf)
                        select_btn.place(x=90, y=27)
                        font_drop = ttk.Combobox(font_frame, textvariable=selected_font, values=font_families, width=35)
                        font_drop.place(x=10, y=5)
                        Label(astrap4_frame, text=font_drop.get()).place(x=0, y=0)
                    else:
                        true_pf_pdf()
                    # astranotebook.tab(3, text="True Power Factor")
    ...
    def true_pf_pdf():
        global export_file_name, selected_directory, selected_file_name, export_dir_name
        if export_dir_name == '':
            export_dir_name = filedialog.asksaveasfilename(title="Export As",
        ...
        header_run = header_para.add_run("InPhase Power Technologies - ASTRA Sizing Document")
        header_run.font.name = f"'{selected_font}'"
        ...
        try:
            ...
            output_cont.font.name = f"'{selected_font}'"
        except:
            pass
        ...
        if p4_comment_box_message.get('1.0', 'end-1c') == placeholder_text:
            ...
            para.font.name = f"'{selected_font}'"
        doc.add_paragraph(''' ''')
        word = "* - Represents User Inputs"
        p = doc.add_paragraph()
        runner = p.add_run(word)
        runner.bold = True
        runner.italic = True
        runner.font.name = f"'{selected_font}'"
        ...
        para = doc.add_paragraph()
        bold_para = para.add_run(''' \n Advanced Power Quality''')
        bold_para.font.name = f"'{selected_font}'"
        section = doc.sections[0]
                    ...
                    if high_pf_entry.get():
                        if lpf <= 1.0 and hpf <= 1.0:
                            if lpf < hpf:
                                ...
                                show_all_btn = Button(astrap4_frame, text="Show all", bg=background_color, fg='black',
                                                      command=show_all)
