...
try:
    ...
    def enable_modified_flag(*args):
        ...
        if (astranotebook.index(astranotebook.select()) == 3):
            ...
            title_p4.config(text="True Power Factor Calculator *")
            location_field.config(text="location :" + str(selected_directory))
            p2_location_field.config(text="location :" + str(selected_directory))
            p3_location_field.config(text="location :" + str(selected_directory))
            p4_location_field.config(text="location :" + str(selected_directory))
    ...
    def export_to_pdf():
        ...
        global number_of_entries, export_dir_name, export_file_name, selected_directory, selected_file_name, unit_diff_cal
            ...
            try:
                if primary_entry.get() and secondary_entry.get():
                    ...
                    if page_flag == 1:
                        ...
                        headings = ['Primary CT', 'Secondary CT']
                        values = [f'{primary_ct_entry.get()}', f'{secondary_ct_entry.get()}']
                        ...
                        doc.add_heading('True Power Factor : All Details :', level=1)

                        # Create table
                        table = doc.add_table(rows=13, cols=5)
                        table.alignment = WD_TABLE_ALIGNMENT.CENTER

                        headings = ['ID', 'Panel Rating', 'Optimal CT Ratio', 'Minimum kVAr', 'Minimum kW']
                        # headings.extend(item[:4] + [item[4], item[5]] for item in data_2[1:])
                        column_widths = [Inches(0.8), Inches(2.3), Inches(2.3), Inches(1.8), Inches(1.8)]
                        ...
                        headings_1 = ['ID', 'Panel Rating', 'Minimum kVAr', 'Minimum kW']
                        ...
                        for i, data in enumerate(minimum_kw, start=1):
                            panel_id, panel_rating, panel_kvar, panel_kw = data
                            # Ensure not to exceed the table size
                            if i < 4:  # Adjusted to 4 rows
                                for j, value in enumerate([panel_id, panel_rating, panel_kvar, panel_kw]):
                                    cell = table_1.cell(i, j)
                                    cell.text = str(value)
                                    # Align text vertically and horizontally
                                    cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
                                    for paragraph in cell.paragraphs:
                        ...
                        # Define headings and values
                        headings = ['Grid kW', 'Panel Rating']
                        values = [f'{grid_kw_entry.get()}', f'{panel_rating_value.get()}']
                        ...
                        # Create a table
                        table_1 = doc.add_table(rows=2, cols=2)
                        table_1.alignment = WD_TABLE_ALIGNMENT.CENTER
                        # Define headings and values
                        headings_1 = ['Max. PF Achieve', 'Unit difference']
                        values_1 = [f'{max_pf['text']}', f'{unit_diff_val['text']}']
                        # Populate table headings and values
                        for a, (heading_1, value_1) in enumerate(zip(headings_1, values_1)):
                            cell_1 = table_1.cell(a, 0)
                            cell_1.text = heading_1
                            cell_1.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
                            cell_1.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
                            cell_1.width = Inches(2)

                            cell_1 = table_1.cell(a, 1)
                            cell_1.text = value_1
                            cell_1.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
                            cell_1.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
                            cell_1.width = Inches(2)
                        ...
                        # Align text horizontally in cells
                        for row_1 in table_1.rows:
                            for cell_1 in row_1.cells:
                                for paragraph_1 in cell_1.paragraphs:
                                    for run_1 in paragraph_1.runs:
                                        run_1.alignment = WD_ALIGN_PARAGRAPH.CENTER
                        # Set table style
                        table_1.style = 'Light Grid Accent 5'
    ...
    def compile_save_data():
        ...
        global minimum_kw, rec_table, unit_diff_cal
        p1_data_entries = []
        p2_data_entries = []
        p3_data_entries = []
        p4_data_entries = []
        ...
        if not rec_tab_val or not any(rec_tab_val):
            p4_data_entries = [["" for _ in range(6)] for _ in range(13)]
            p4_sugg_entries = [["" for _ in range(4)] for _ in range(3)]
        else:
            try:
                for x in range(len(rec_tab_val)):
                    for y in range(len(rec_tab_val[0])):
                        p4_data_entries.append(rec_tab_val[x][y])
                for x in range(len(minimum_kw)):
                    p4_sugg_entries.append(minimum_kw[x])
                    print("Sugg : ", p4_sugg_entries)
            except Exception as e:
                print(f"Error: {e}")
                '''p4_data_entries = [["" for _ in range(len(rec_tab_val[0]))] for _ in range(len(rec_tab_val))]
                p4_sugg_entries = [["" for _ in range(len(minimum_kw[0]))] for _ in range(len(minimum_kw))]'''
        ...
        sizing_data = {
            ...
            'p4_primary value': primary_ct_entry.get(),
            'p4_secondary value': secondary_ct_entry.get(),
            'p4_data values': p4_data_entries,
            'p4_suggestion values': p4_sugg_entries,
            'max_pf_achieve': max_pf['text'],
            'unit_difference': unit_diff_val['text'],
            'grid_kw': grid_kw_entry.get(),
            'panel_rating_combo': panel_choosen.current(),
        }
        print("P4_Suggestion value : ", p4_sugg_entries)
        print("P4_Entries value : ", p4_data_entries)
        #Label(astrap4_frame, text = sizing_data['panel_rating_combo']).place(x=30, y=300)
    ...
    def clear_contents():
        ...
        if (astranotebook.index(astranotebook.select()) == 3):
            # Deleting the pervious loaded status
            if page_flag == 1:
                primary_ct_entry.config( state="normal")
                primary_ct_entry.delete(0, "end")
                secondary_ct_entry.config(state="normal")
                secondary_ct_entry.delete(0, "end")
            elif page_flag == 0:
                grid_kw_entry.config(state = "normal")
                grid_kw_entry.delete(0,"end")
                panel_choosen.current(0)
    ...
    def import_nfo_data():
        ...
        global page2_frequency, page3_frequency, import_progress_flag, page_flag, panel_choosen
        ...
        if page_flag == 1:
            ...
            grid_kw_entry.delete(0, "end")
            panel_choosen.config(state='disabled')
            panel_choosen.current(0)
        else:
            grid_kw_entry.config(state='normal')
            grid_kw_entry.delete(0, "end")
            panel_choosen.config(state='normal')
            panel_choosen.current(0)

            primary_ct_entry.config(state="disabled")
            primary_ct_entry.delete(0, "end")
            secondary_ct_entry.config(state="disabled")
            secondary_ct_entry.delete(0, "end")
        ...
        #-------------------------- 4th Page ---------------------------#
                global p4_data_entries, table_frame
                ...
                if page_flag == 1:
                    ...
                    for x in range(0, 2):
                        ...
                        if (x == 0):
                            primary_ct_entry.config(state='normal')
                            primary_ct_entry.delete(0, "end")
                            primary_ct_entry.insert(0, str(imported_data['p4_primary value']))
                            primary_ct_entry.config(state='disable')
                            print("Done - 2")
                        else:
                            primary_ct_entry.delete(0, "end")
                            primary_ct_entry.insert(0, str(imported_data['p4_primary value']))
                    ...
                    secondary_ct_entry.config(state='normal')
                    secondary_ct_entry.delete(0, "end")
                    secondary_ct_entry.insert(0, str(imported_data['p4_secondary value']))
                    secondary_ct_entry.config(state='normal')

                    # Get the data from 'imported_data'
                    imported_opt = imported_data['p4_data values']
                    first_comp = imported_opt[0]
                    ...
                    num_cols = 5  # Calculate the number of columns
                                # Get the data from 'imported_data'
                    ...
                    try:
                        ...
                        sugg_table = np.array([]).reshape(4, 4)
                        try_1 = 0  # Set try_1 to 0 in case of error

                    # Check imported_sugg condition and set try_1 accordingly
                    if imported_sugg[1] == ["", "", "", ""]:
                        try_1 = 0
                    else:
                        try_1 = 1
                    ...
                    if try_1 == 1:
                        try:
                            for table_frame_1 in table_frame.winfo_children():
                                table_frame_1.destroy()
                        except:
                            pass
                        def del_table():
                            global toggle_btn, popup_frame, primary_ct_entry, secondary_ct_entry, calculate_btn
                            popup_frame.destroy()
                        ...
                        def all_details_pop_up():
                            global rec_tab_val, toggle_btn, popup_frame, primary_ct_entry, secondary_ct_entry, calculate_btn
                            popup_frame = Frame(astrap4_frame, bg=background_color)
                            popup_frame.place(x=125, y=195)
                            ...
                            try:
                                ...
                                opt_table = np.array(imported_opt).reshape(13, 6)
                            # Iterate over the reshaped data to populate GUI elements
                            for x, row in enumerate(opt_table):
                                for y, item in enumerate(row):
                                    if isinstance(item, int) or isinstance(item, str):
                                        font_style = ("Arial", 13, "bold") if x == 0 else ("Arial", 12)
                                        bg_style = "wheat" if x == 0 else background_color
                                        label = Label(popup_frame, text=str(item), width=20, anchor="center",bg=bg_style, font=font_style, borderwidth=1, relief="solid")
                                        label.grid(row=x, column=y, padx=0, pady=0, sticky="nsew")

                            dummy_lab = Label(popup_frame, text="").grid(row=13)
                            # Add a button to close the window
                            close_button = Button(popup_frame, text="OK", font=("Arial", 12, "bold"), command=del_table)
                            close_button.grid(row=14, columnspan=5)

                        table_frame = Frame(astrap4_frame, height=200, bg=background_color)
                        table_frame.place(x=100, y=335)

                        # Add the label "Optimal Panel Rating Based on Optimum kW"
                        headings = [("Panel ID", 15), ("Panel Rating", 25), ("Optimal kVAr", 20), ("Optimal kW Ratio", 20)]
                        ...
                        for a, row_1 in enumerate(sugg_table,start=2):  # Start at row 2 to skip the headings row and the label row
                            for b, item_1 in enumerate(row_1):
                                if isinstance(item_1, int) or isinstance(item_1, str):
                                    ...
                                    imported_val = Label(table_frame, text=str(item_1), width=headings[b][1], anchor="center", font=font_style, bg=background_color, relief='ridge', borderwidth=2)
                                    imported_val.grid(row=a + 1, column=b)
                        dummy = Label(table_frame, text=" ", bg=background_color)
                        dummy.grid(row=6)
                        all_details = Button(table_frame, text="All Details", bg=background_color, command=all_details_pop_up)
                        all_details.grid(row=7, columnspan=4)
                    else:
                        headings = ["", "", "", ""]
                        pass # Empty headings or configure as needed
                elif page_flag ==0:
                    grid_kw_entry.config(state='normal')
                    grid_kw_entry.delete(0, "end")
                    grid_kw_entry.insert(0, str(imported_data['grid_kw']))
                    grid_kw_entry.config(state='normal')
                    #Label(astrap4_frame, text=imported_data['panel_rating_combo']).place(x=30, y=500)
                    if (str(imported_data['panel_rating_combo']) == '0'):
                        panel_choosen.current(0)
                    elif (str(imported_data['panel_rating_combo']) == '1'):
                        panel_choosen.current(1)
                    elif (str(imported_data['panel_rating_combo']) == '2'):
                        panel_choosen.current(2)
                    elif (str(imported_data['panel_rating_combo']) == '3'):
                        panel_choosen.current(3)
                    elif (str(imported_data['panel_rating_combo']) == '4'):
                        panel_choosen.current(4)
                    elif (str(imported_data['panel_rating_combo']) == '5'):
                        panel_choosen.current(5)
                    elif (str(imported_data['panel_rating_combo']) == '6'):
                        panel_choosen.current(6)
                    elif (str(imported_data['panel_rating_combo']) == '7'):
                        panel_choosen.current(7)
                    elif (str(imported_data['panel_rating_combo']) == '8'):
                        panel_choosen.current(8)
                    elif (str(imported_data['panel_rating_combo']) == '9'):
                        panel_choosen.current(9)
                    elif (str(imported_data['panel_rating_combo']) == '10'):
                        panel_choosen.current(10)
                    elif (str(imported_data['panel_rating_combo']) == '11'):
                        panel_choosen.current(11)
                    elif (str(imported_data['panel_rating_combo']) == '12'):
                        panel_choosen.current(12)

                    max_pf.config(text=str(imported_data['max_pf_achieve']))
                    unit_diff_val.config(text=str(imported_data['unit_difference']))
    ...
    def validate_licensing():
        ...
        if (validate_license_flag == 1):
            ...
            status_p3_entry.config(borderwidth=2, state="normal")
            status_p3_entry.delete(0, "end")
            status_p3_entry.insert(0, "Visit licensing to proceed")
            status_p3_entry.config(borderwidth=2, state="disable")
            ...
            try:
                last_vali_p1 = Label(astrap1_frame, text=f"Last Validity : {datetime.strptime(lic_validity, "%Y-%m-%d").strftime("%d-%b-%Y")}", font=("Arial", 13), fg="red",bg=background_color)
                last_vali_p1.place(relx=0.95, rely=0.06, anchor=NE)
                last_vali_p2 = Label(astrap2_frame, text=f"Last Validity : {datetime.strptime(lic_validity, "%Y-%m-%d").strftime("%d-%b-%Y")}", font=("Arial", 13), fg="red",bg=background_color)
                last_vali_p2.place(relx=0.95, rely=0.06, anchor=NE)
                last_vali_p3 = Label(astrap3_frame, text=f"Last Validity : {datetime.strptime(lic_validity, "%Y-%m-%d").strftime("%d-%b-%Y")}", font=("Arial", 13), fg="red", bg=background_color)
                last_vali_p3.place(relx=0.95, rely=0.06, anchor=NE)
                last_vali_p4 = Label(astrap4_frame, text=f"Last Validity : {datetime.strptime(lic_validity, "%Y-%m-%d").strftime("%d-%b-%Y")}", font=("Arial", 13), fg="red", bg=background_color)
    ...
    global max_pf_ach, unit_diff

    table_frame = Frame(astrap4_frame, height = 200, bg=background_color)
    table_frame.place(x=100, y=335)

    title_p4 = Label(astrap4_frame, text="True Power Factor Calculator", font=("Arial", 15, "bold"), bg=background_color)
    title_p4.place(x=480, y=5)
    # ------------------- Label for Input -----------------#
    primary_ct = Label(astrap4_frame, text="Primary CT", bg=background_color, font=("Arial", 12))
    primary_ct.place(x=470, y=45)
    secondary_ct = Label(astrap4_frame, text="Secondary CT", bg=background_color, font=("Arial", 12))
    secondary_ct.place(x=470, y=80)
    grid_kw = Label(astrap4_frame, text="Grid kW", bg=background_color, font=("Arial", 12))
    grid_kw.place(x=470, y=115)
    panel_rating = Label(astrap4_frame, text="Panel Rating", bg=background_color, font=("Arial", 12))
    panel_rating.place(x=470, y=150)
    toggle_label = Label(astrap4_frame, text="Switch for custom CT", bg=background_color, font=("Arial", 12))
    toggle_label.place(x=850, y=45)

    status_p4_entry=Label(astrap4_frame, text="", font=("Arial", 14), bg = background_color, fg = "red")
    status_p4_entry.place(relx=0.498, rely=0.94, anchor="s")

    pri_less_sec = Label(astrap4_frame, text="", font=("Arial", 14), bg=background_color, fg="red")
    pri_less_sec.place(x=370, y=224)

    # ---------------- Input Entries and Dropdown --------------#
...
#------------------------------------ Input entries and drop down -------------------------------------#
    primary_entry = StringVar()
    secondary_entry = StringVar()
    kw_entry = StringVar()

    primary_ct_entry = Entry(astrap4_frame, font=("Arial", 12), textvariable = primary_entry,borderwidth=0, bg="White", validate="key",validatecommand=(validation, '%P'), highlightthickness=2, highlightbackground = "paleturquoise", highlightcolor= "paleturquoise")
    primary_ct_entry.place(x=590, y=47)
    secondary_ct_entry = Entry(astrap4_frame, font=("Arial", 12), textvariable = secondary_entry, borderwidth=0, bg="White", validate="key", validatecommand=(validation, '%P'), highlightthickness=2, highlightbackground = "paleturquoise", highlightcolor= "paleturquoise")
    secondary_ct_entry.place(x=590, y=82)
    grid_kw_entry = Entry(astrap4_frame, font=("Arial", 12), textvariable = kw_entry, borderwidth=0, bg="White", validate="key", validatecommand=(validation, '%P'), highlightthickness=2, highlightbackground = "paleturquoise", highlightcolor= "paleturquoise")
    grid_kw_entry.place(x=590, y=117)
    # -------------------- Drop Down ----------------------#
    panel_rating_value = StringVar()
    panel_choosen = ttk.Combobox(astrap4_frame, font=("Arial", 12), width=18, textvariable=panel_rating_value,state="readonly")
    # Adding combobox drop down list
    panel_choosen['values'] = ('Select',
                               ' 630A ASTRA - B03',
                               ' 630A ASTRA - B05',
                               ' 630A ASTRA - B10',
                               ' 315A ASTRA - B03',
    ...
    panel_choosen.place(x=590, y=152)
    panel_choosen.current(0)

    def update_results(*args):
        global max_pf, unit_diff_val, table_frame, panel_rating_value, title_p4
        max_pf.config(text="")
        unit_diff_val.config(text="")
        pri_less_sec.config(text="")
        status_p4_entry.config(text="")
        astranotebook.tab(3, text="True Power Factor*")
        title_p4.config(text="True Power Factor Calculator *")
        try:
            ...
            pass
    primary_entry.trace_add('write', update_results)
    secondary_entry.trace_add('write', update_results)
    kw_entry.trace_add('write', update_results)
    panel_rating_value.trace('w', update_results)

    def update_label():
        global choosen, custom_kvar, max_pf
        selected_value = panel_choosen.current()
    ...
    panel_choosen.bind("<<ComboboxSelected>>", lambda event: update_label())

    def btn(value):
        global page_flag,table_name
        if toggle_var.get() == 0:
            max_pf_ach.config(text="Maximum PF Achieve")
            unit_diff.config(text="Unit Difference per Day")
            table_name.config(text="")
            primary_ct_entry.delete(0, "end")
            primary_ct_entry.config(state="disabled")
            secondary_ct_entry.delete(0, "end")
            secondary_ct_entry.config(state="disabled")
            ...
            panel_choosen.current(0)
            panel_choosen.config(state="disabled")
            table_name = Label(astrap4_frame, text="Suggestion Table", font=("Arial", 16, 'bold'), bg=background_color, fg="black")
            table_name.place(x=530, y=275)
            page_flag = 1
    ...
    def cal_btn():
        global custom_kvar, panel_choosen, selected_value, max_pf, unit_diff_val

        if page_flag == 1:
            try:
                if not primary_ct_entry.get():
                    messagebox.showerror("Error!", "Enter primary values")
                elif not secondary_ct_entry.get():
                    messagebox.showerror("Error!", "Enter secondary values")
                elif not primary_ct_entry.get() and not secondary_ct_entry.get():
                    messagebox.showerror("Error!", "Enter both values")

                pri_val = int(primary_ct_entry.get())
                sec_val = int(secondary_ct_entry.get())

                if pri_val > 0 and sec_val > 0:
                    if pri_val > sec_val:
                        cal_opt_val = round(pri_val / sec_val)
                        custom_ct(pri_val, cal_opt_val)
                        #condition_1(pri_val, sec_val, cal_opt_val)
                    else:
                        swap_proceed = messagebox.askyesnocancel('Conformation',
                                                                 "Warning: primary value is less than secondary value.\n\tDo you want to swap the values?")
                    ...
                    messagebox.showerror('Error', "Values should br greater than 0")
            except:
                pass
        elif page_flag == 0:
            try:
                if not grid_kw_entry.get():
                    messagebox.showerror("Error!", "Enter Grid kW values")
                elif panel_choosen.get() == "Select":
                    messagebox.showerror("Error!", "Select the Panel Rating")

                kw_val = int(grid_kw_entry.get())
                custom_kva = math.sqrt((custom_kvar**2) + (kw_val**2))
                custom_pf = round(kw_val/custom_kva, 3)
                max_pf.config(text=custom_pf)
                unit_diff_cal = kw_val / custom_kva
                unit_diff_val.config(text=unit_diff_cal)
            except:
                pass

    def custom_ct(pri_val, cal_opt_val):
        global rows, columns
        global minimum_kw, max_pf
        global rec_tab_val
        global rec_table
        ...
        try:
            for table_frame_3 in table_frame.winfo_children():
                table_frame_3.destroy()
        except:
            pass
        opt_ct_ratio = [
            ["Panel ID", "Panel Rating", "Optimal CT Ratio", "Min. kVAr", "Min. kW"],
        ...
        for row_idx, row in enumerate(rec_tab_val[1:], start=1):
            ...
            if data_val < cal_opt_val:
                ...
                if row_idx < len(opt_kw):
                    row[3:4] = " - ", opt_kw[row_idx][0]

        # WATCH
        for x in range(len(rec_tab_val)):
            for y in range(len(rec_tab_val[0])):
                opt = rec_tab_val[x][y]
                #Label(astrap4_frame, text=opt, width=10, anchor="center", bg=background_color, borderwidth=1, relief="solid").grid(row=x, column=y, padx=0, pady=0, sticky="nsew")
        page_2()
    ...
    def all_details_pop_up():
        ...
        for x in range(len(rec_tab_val)):
            for y in range(len(rec_tab_val[0])):
                font_style = ("Arial", 13, "bold") if x == 0 else ("Arial", 12)
                bg_style = "wheat" if x == 0 else background_color
                label = Label(popup_frame, text=rec_tab_val[x][y], width=20, anchor="center", bg=bg_style, font=font_style, borderwidth=1, relief="solid").grid(row=x, column=y, padx=0, pady=0, sticky="nsew")
    ...
    # ------------------ Buttons -----------------#
    toggle_var = IntVar()
    toggle_btn = Scale(astrap4_frame,orient=HORIZONTAL, variable=toggle_var, length=50, from_=0, to=1, showvalue=False, sliderlength=25, command=lambda val: btn(val))
    toggle_btn.place(x=895, y=70)
    #start_color = (51, 153, 255)  # Light blue
    #end_color = (0, 102, 204)  # Dark blue

    calculate_btn = Button(astrap4_frame, text="Calculate", font=("Arial", 13, "bold"), foreground="White", background="#4584b6", command = cal_btn, bd=0)
    calculate_btn.place(x=550, y=187)

    #--------------------------- Output Label --------------------------------#
    max_pf_ach = Label(astrap4_frame, text = "Maximum PF Achieve", font = ("Arial", 14, 'bold'), bg=background_color)
    max_pf_ach.place(x=470, y=245)
    max_pf = Label(astrap4_frame, text="", font=("Arial", 12), bg = background_color)
    max_pf.place(x=700, y=247)
    unit_diff = Label(astrap4_frame, text = "Unit Difference per Day", font = ("Arial", 14, 'bold'), bg=background_color)
    unit_diff.place(x=470, y=302)
    unit_diff_val = Label(astrap4_frame, text = "", font = ("Arial", 12), bg=background_color)
    unit_diff_val.place(x=700, y=304)

    def page_1():
        global max_pf_ach, unit_diff, table_frame
        max_pf_ach.config(text="Maximum PF Achieve")
        unit_diff.config(text="Unit Difference per Day")
        try:
            for widget in table_frame.winfo_children():
            ...
            pass
    def page_2():
        global max_pf_ach, unit_diff, minimum_kw, first
        ...
        minimum_kw = []
        sort_column = sorted(rec_tab_val, key=lambda x: str(x[4]))
        first = sort_column[:3]
        for row in first:
            if str(row[4]) in [str(item[4]) for item in first]:
                minimum_kw.append((row[0], row[1], row[3], row[4]))

        # Create empty cells below each column
        for i in range(len(minimum_kw)):
            for j in range(len(minimum_kw[i])):
                label = Label(table_frame, text=minimum_kw[i][j], width=headings[j][1], bg=background_color, fg='black',
        ...
        dummy.grid(row = 5)
        all_details = Button(table_frame, text="All Details", bg=background_color, command=all_details_pop_up)
        all_details.grid(row = 6, columnspan = 4)
