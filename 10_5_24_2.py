    ...
    try:
        ...
        try:
            ...
            for x, row in enumerate(opt_table):
                for y, item in enumerate(row):
                    if isinstance(item, int) or isinstance(item, str):
                        ...
                        rec_table = Label(frame_4, text=str(item), width=20, anchor="center", font=font_style,
                                          borderwidth=1, relief="solid", bg=background_color)
                        rec_table.grid(row=x, column=y, padx=0, pady=0, sticky="nsew")

                        # Get the data from 'imported_data'
            imported_sugg = imported_data['p4_suggestion values']
            print("Imported sugg : ", imported_sugg)
            ...
            if try_1 == 1:
                ...
                headings = ["Panel ID", "Panel Rating","Minimum kVAr", "Minimum kW"]
            else:
                # Add an empty label since reshaping failed
                Label(frame_6, text="", font=("Arial", 10, "bold"), bg=background_color).grid(row=0, column=0,
                                                                                              columnspan=num_cols_1,
                                                                                              padx=9, pady=1)
                headings = ["", "", "", ""]  # Empty headings

            # Create and place the headings labels in the GUI
            for col, heading in enumerate(headings):
                Label(frame_6, text=heading, width=15, anchor="center", font=("Arial", 11, "bold"),
                      bg=background_color).grid(row=1, column=col, padx=3, pady=5, sticky="nsew")
            ...
            for a, row_1 in enumerate(sugg_table, start=2):  # Start at row 2 to skip the headings row and the label row
                for b, item_1 in enumerate(row_1):
                    if isinstance(item_1, int) or isinstance(item_1, str):
                        font_style = ("Arial", 10)
                        Label(frame_6, text=str(item_1), width=15, anchor="center", font=font_style,
                              bg=background_color).grid(row=a, column=b, padx=3, pady=5, sticky="nsew")
...
def validate_licensing():
    ...
    if (validate_license_flag == 1):
        ...
        last_vali_p1 = Label(astrap1_frame, text=f"Last Validity : {lic_validity}", font=("Arial", 13), fg = "red", bg=background_color).place(relx = 0.97, rely = 0.06, anchor=NE)
        last_vali_p2 = Label(astrap2_frame, text=f"Last Validity : {lic_validity}", font=("Arial", 13), fg = "red", bg=background_color).place(relx = 0.97, rely = 0.06, anchor=NE)
        last_vali_p3 = Label(astrap3_frame, text=f"Last Validity : {lic_validity}", font=("Arial", 13), fg = "red", bg=background_color).place(relx = 0.97, rely = 0.06, anchor=NE)
        last_vali_p4 = Label(frame_1, text=f"Last Validity : {lic_validity}", font=("Arial", 13), fg = "red", bg=background_color).place(relx = 0.97, rely = 0.3, anchor=NE)
...
def submit():
    ...
    try:
        ...
        pri_val = int(primary_entry.get())
        sec_val = int(secondary_entry.get())
...
def condition_1(pri_val, sec_val, cal_opt_val):
    ...
    for row in sort_column:
        if str(row[5]) in [str(item[5]) for item in first]:
            minimum_kw.append((row[0], row[1], row[4], row[5]))
    ...
    sugg_lable_head_5 = Label(table_frame, text="Minimun kVAr", font=("Arial", 10, "bold"), bg=background_color, width=15)
    sugg_lable_head_5.grid(row=1, column=2, padx=5, pady=3)

    sugg_lable_head_4 = Label(table_frame, text="Minimum kW", font=("Arial", 10, "bold"), bg=background_color, width=15)
    sugg_lable_head_4.grid(row=1, column=3, padx=5, pady=3)

    row_index = 2
    for i in range(min(3, len(minimum_kw))):
        panel_id = minimum_kw[i][0]
        panel_rating = minimum_kw[i][1]
        panel_kvar = minimum_kw[i][2]
        panel_kw = minimum_kw[i][3]
        pan_rat = (panel_id, panel_rating, panel_kvar, panel_kw)

        category, data,min_kvar, opt_ct_ratio = pan_rat
        ...
        sugg_pan_kvar = Label(table_frame, text=str(min_kvar), font=("Arial", 10), bg=background_color, width=15).grid(row=row_index, column=2, padx=5, pady=5)
        sugg_pan_kw = Label(table_frame, text=str(opt_ct_ratio), font=("Arial", 10), bg=background_color,width=15).grid(row=row_index, column=3, padx=5, pady=5)
        row_index += 1
...
def update_results(*args):
    ...
    global rec_tab_val, minimum_kw, rec_table
    print(pri_less_sec)
...
# Labels
head_notebook = Label(frame_1, text="True Power Factor Performance Calculator  ", bg=background_color, fg="Black",
                      font=("Arial", 17))
head_notebook.pack(padx=10, pady=10)
pri_ent_label = Label(frame_1, text="Primary value", bg=background_color, fg="Black", font=("Arial", 11)).place(
    relx=0.35, rely=0.48, anchor=W)
primary_entry = Entry(frame_1, textvariable=primary, font=("Arial", 11), validate="key", validatecommand=(validation, '%P'))
primary_entry.place(relx=0.52, rely=0.48, anchor=W)

sec_ent_label = Label(frame_1, text="Secondary value", bg=background_color, fg="Black", font=("Arial", 11)).place(
    relx=0.35, rely=0.75, anchor=W)
secondary_entry = Entry(frame_1, textvariable=secondary, font=("Arial", 11), validate="key", validatecommand=(validation, '%P'))
