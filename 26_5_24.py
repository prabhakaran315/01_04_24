...
try:
    ...
    def export_to_pdf():
        ...
        if (astranotebook.index(astranotebook.select()) == 3):
            ...
            show_button()
            try:
                if low_pf_entry.get() and high_pf_entry.get() and grid_kw_entry.get() and panel_rating_combo.current() != 0:
                    # astranotebook.tab(3, text="True Power Factor")
                    ...
                    if export_dir_name == '':
                        export_dir_name = filedialog.asksaveasfilename(title="Export As", filetypes=(
                        ("PDF", "*.docx"), ("All Files", "*.*")), initialfile="Design Document")
                        try:
                            # status_p4_entry.config(text="Export initiated...")
                            os.chdir(os.path.dirname(export_dir_name))
                            export_file_name = os.path.basename(export_dir_name).split('.', 1)[0]
                    ...
                    headings = ['Model', 'Grid kW value', 'Min. PF', 'Max. Pf', 'Unit diff./day (low)',
                                'Unit diff./day (high)']
                    ...
                    doc.add_heading('True Power Factor :', level=1)
    ...
    # astranotebook.add(astrap5_frame, text="All Details", state='disabled')
    ...
    def import_nfo_data():
        ...
        try:
            ...
            try:
                ...
                unit_diff_entry_2.insert(0, str(imported_data['unit_entry_2']))
                ...
                if out_lab:
                    ...
                    output_content_1.config(fg='black', state='disabled')
                    # Label(astrap4_frame, text=out_lab).place(x=10, y=200)

                # Label(astrap4_frame, text = str(imported_data['out_put_content'])).place(x=10, y=100)
                # Label(astrap4_frame, text = imported_sugg).place(x=10, y=20)
                ...
                for index, (heading, width) in enumerate(headings):
                    label = Label(table_frame, text=heading, width=width, fg='black', bg="wheat",
    ...
    grid_kw_entry = Entry(astrap4_frame, width=7, font=("Verdana", 14), validate="key", textvariable=grid_kw_val,
    ...
    def suggestion_tab():
        ...
        headings = [("Panel ID", 15), ("Panel Rating", 20), ("Power Factor", 20), ("Unit Diff / day", 20), ("No. Panel", 15)]

        # Create labels for headings with custom widths
        for index, (heading, width) in enumerate(headings):
            label = Label(table_frame, text=heading, width=width, fg='black', bg="wheat", font=('Arial', 16, 'bold'),
        ...
        for row_idx, row in enumerate(rec_tab_val[0:], start=0):
            ...
            numeric_part = ''.join(filter(str.isdigit, row[1]))

            # Convert the extracted value to an integer directly
            panel_count_1 = int(numeric_part[:3])
            panel_count = math.ceil(kw_val/panel_count_1)
            row[2:4] = pf, unit_diff_value, panel_count
            all_label.append((row[0], row[1], row[2], row[3], row[4]))

            # Label(astrap4_frame, text=all_label).place(x=10, y=10*3)
            # Check the power factor against thresholds
            if pf >= lpf or pf >= hpf:
                row[2:4] = pf, unit_diff_value, panel_count
                sugg.append((row[0], row[1], row[2], row[3], row[4]))
        while len(sugg) < 3:
            sugg.append(["NA", "NA", "NA", "NA", "NA"])
        ...
        for row in first:
            if str(row[3]) in [str(item[3]) for item in first]:
                minimum_kw.append((row[0], row[1], row[2], row[3], row[4]))

        # Create empty cells below each column
        for i in range(len(minimum_kw)):
            for j in range(len(minimum_kw[i])):
                sugg_label = Label(table_frame, text=minimum_kw[i][j], width=headings[j][1], bg=background_color,
                                   fg='black',
                                   font=('Arial', 16, 'bold'), relief='ridge', borderwidth=2)
                    ...
                    if high_pf_entry.get():
                        if lpf <= 1.0 and hpf <= 1.0:
                            if lpf < hpf:
                                ...
                                unit_diff_value_1 = float(round((kva_1 - kw_val) * 24, 3))
                                ...
                                if pf >= str(hpf):
                                    ...
                                    unit_diff_entry_2.config(state='normal')
                                    unit_diff_entry_2.delete(0, "end")
                                    unit_diff_entry_2.insert(0, unit_diff_value_2)
                                    unit_diff_entry_2.config(state='disabled')
    ...
    for i in range(3):
        for j in range(5):
            label = Label(table_frame, text="", width=headings[j][1], bg=background_color, fg='black',
                          font=('Arial', 16, 'bold'), relief='ridge', borderwidth=2)
    ...
    output_content_1 = Text(astrap4_frame, height=3, width=91, font=("Verdana", 12), wrap='word')
    ...
    def show_all():
        ...
        all_table.place(x=40, y=100)

        try:
            # Define headings and their respective widths
            headings = [("Panel ID", 10), ("Panel Rating", 20), ("Power Factor", 15), ("Unit Diff / day", 16), ("No. Panels", 15)]
            # Create labels for headings with custom widths
            for index, (heading, width) in enumerate(headings):
