...
try:
    ...
    #----- Heading -----#
    head_label = Label(astrap4_frame, text = "True Power Factor Based Panel Selection", font = ("Verdana", 16), bg=background_color)
    head_label.place(x=400, y=12)
    ...
    panel_rating_combo = ttk.Combobox(astrap4_frame, font=("Arial", 12), width=18, textvariable=panel_rating_value, state="readonly")
    ...
    panel_rating_combo.place(x=270, y=59)
    ...
    def validate_input_1(new_text_1):
        if not new_text_1:
            return True
        try:
            value = float(new_text_1)
            if value <= 1.0:
                return True
            else:
                messagebox.showwarning("Warning", "PF value should be less than 1.0")
                return False
        except ValueError:
            return False

    validation_1 = root.register(validate_input_1)

    grid_kw_entry = Entry(astrap4_frame, width = 10, font = ("Verdana", 14), validate="key", textvariable = grid_kw_val, bg=background_color, validatecommand=(validation, '%P'))
    grid_kw_entry.place(x=270, y=107)

    low_pf_entry = Entry(astrap4_frame, width = 7, font = ("Verdana", 14), validate="key", textvariable = lpf_val, bg=background_color, validatecommand=(validation_1, '%P'))
    low_pf_entry.place(x=270, y=177)

    high_pf_entry = Entry(astrap4_frame, width = 7, font = ("Verdana", 14), validate="key", textvariable = hpf_val, bg=background_color, validatecommand=(validation_1, '%P'))
    high_pf_entry.place(x=400, y=177)

    #----------- Button function -----------#
    def update_label():
        global panel_rating_combo, custom_kvar, max_pf
    ...
    def show_button():
        if panel_rating_combo.current() != 0:
            if grid_kw_entry.get():
                if low_pf_entry.get() and high_pf_entry.get():
                    if low_pf_entry.get() < high_pf_entry.get():
                        kw_val = int(grid_kw_entry.get())
                        kva = math.sqrt((custom_kvar ** 2) + (kw_val ** 2))
                        pf = str(round(kw_val / kva, 3))

                        if pf <= low_pf_entry.get():
                            output_content.config(text="The desired power factor of"f" Low Power Factor : {low_pf_entry.get()}"" can be achieved with a panel rated "
                                                       "at"f" {panel_rating_combo.get()}.\nThis configuration will efficiently handle the required load of"f" {pf}.")
                        elif pf <= high_pf_entry.get():
                            output_content.config(text="The desired power factor of"f" High Power Factor : {high_pf_entry.get()}"" can be achieved with a panel rated "
                                     "at"f" {panel_rating_combo.get()}.\nThis configuration will efficiently handle the required load of"f" {pf}.")
                            unit_diff.config(text="Unit Difference per day : "+pf)
                        else:
                            output_content.config(
                                text="The desired power factor not achieved with a panel rated at"f" {panel_rating_combo.get()}."
                                     f"This configuration will not\nefficiently handle the required load of "f"{pf}. So, Suggestion is given below check it.")
                            unit_diff.config(text="Unit Difference per day : "+pf)
                    else:
                        messagebox.showwarning('Power Factor', "High PF should be higher than Low PF")
                else:
                    messagebox.showwarning('Power factor', "Enter the both values")
            else:
                messagebox.showwarning("Grid value", "Enter the kW value")
        else:
            messagebox.showwarning("Panel Rating", 'Select the panel rating')
    #----------- Buttons -----------#
    check_btn = Button(astrap4_frame, text = "Show", font = ("Verdana", 12), bg=background_color, command = show_button)
    check_btn.place(x=345, y=215)
    # -----------------------------------------------------------------------------------------------------------------------------------------------------------#
